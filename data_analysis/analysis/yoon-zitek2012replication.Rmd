...
raw.data.path <- "/Users/ericang/Documents/Courses/Psych254/zt12rp/data_analysis/raw_data/"
```
...
p1 <- ggplot(data = rp,
             aes(x=cond, y=rt, fill=expt, colour=expt)) +
    geom_dotplot(binaxis = "y", stackdir = "center") +
    geom_errorbar(aes(ymin=rt-cil, ymax=rt+cih),
                  width=.2) +
    guides(colour=FALSE) +
    guides(fill=guide_legend("Experiment")) +
    xlab("Condition") + 
...
jso_mss$cond <- as.factor(jso_mss$cond)
jso_mss$cond <- relevel(jso_mss$cond, "ds")
lmer <- lmer(inv_rt ~ cond + (cond | subid) + (1 | item), data = jso_mss)
summary(lmer)
```
